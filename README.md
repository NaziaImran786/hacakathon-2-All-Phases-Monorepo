...
Returns success message only: {"message": "User created successfully"}
Frontend redirects to /login with toast message
User must explicitly log in with credentials
Login Flow:

User submits credentials to /token endpoint
Backend validates and returns JWT (30-minute expiry)
...
Local Development: Run directly on your machine (Python + Node.js)
Phase IV: Kubernetes: Deploy to Minikube/Kubernetes with Helm charts
Local Development
Prerequisites
Python 3.9+
Node.js 18+
...
cd frontend
npm install
Configure environment: Create .env.local file in frontend/:
...
Type: "Add a task called 'Buy groceries'"
AI will confirm and create task
...
Kubernetes Specification: specs/features/kubernetes-deployment.md
Validation Script: scripts/validate-deployment.sh
E2E Test Script: scripts/test-e2e-connectivity.sh
Contributing
Fork the repository
